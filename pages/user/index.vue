<template>
  <div class="user-admin-page">
    ...
    <el-drawer
      ...
      :visible.sync="showDialogCustomer"
      size="30%"
    ...
    >
      <div class="" style="min-height: 45px">
        ...
        >
          ...
          <div @click="showDialogCustomer = false">
            <el-icon
        ...
        >
          <div style="width: 100%; padding: 20px">
            <p>
              <strong>Họ và tên: </strong> {{ userCustomerDetail.fullName }}
            </p>
            <p>
              <strong>Số điện thoại: </strong
              >{{ userCustomerDetail.phoneNumber }}
            </p>
            <p><strong>Email: </strong>{{ userCustomerDetail.email }}</p>
            <label><strong>Avatar</strong></label
            ><br />
            <img
              style="width: 90%; object-fit: cover; margin-top: 10px"
              :src="userCustomerDetail.avatar"
            />
          </div>
        </el-scrollbar>
      </div>
  ...
  methods: {
    async getData() {
      ...
      await apiServices
        ...
        .then((res) => {
          if (res) {
            _this.userCustomer = res?.data?.data.filter(
              (e) => e?.role === 'USER'
            )
            _this.loading = false
          }
